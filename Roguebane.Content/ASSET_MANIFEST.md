...
`icons/minion/{skeleton,golem,hound}` (payload B18 — golem + hound added 2026-07-12, same flat-pictogram register), `ui/reticle/{focus,focus_p0,focus_p1,focus_p2,secondary,aiming}`. Re-run to reproduce identically.
...
| `ui/button/button_pager` | compact SQUARE pager button (Merchant + Equipment-inventory page flip); ◀/▶ glyph drawn at runtime over it | Merchant, Equipment | page prev/next | 88×68 | 1 | hi-fi · idle skin from `proto/ui_gen.js` `buildPager` |
